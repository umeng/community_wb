...
public class MainIndicator extends LinearLayout{
     ...
     * 标题选中时的颜色
     */
    protected static  int COLOR_TEXT_HIGHLIGHTCOLOR ;
...
//
//        // 绘制tab的下划线
//        canvas.drawLine(mTranslationX, getHeight(), mTranslationX
//                + mHalfScreenWidth, getHeight(), mLinePaint);
//        // 绘制一行的分割线
...
//                mHalfScreenWidth = getChildAt(0).getWidth();
//            }else
            mHalfScreenWidth = ((getScreenWidth()- CommonUtils.dip2px(getContext(), 50)) / mTabVisibleCount)/10*7;
        }
    ...
    public void scroll(int position, float offset) {
         ...
         */
        // 不断改变偏移量，invalidate
        mTranslationX = (getScreenWidth()-CommonUtils.dip2px(getContext(), 80)) / 4 * (position + offset)+(getScreenWidth()-CommonUtils.dip2px(getContext(), 80))/20*3/4;
//        mTranslationX = (getWidth()) / 4 * (position + offset);
